...
export async function POST(req:NextRequest){
  ...
  if(memberError||(!members?.length&&!isAdmin))return NextResponse.json({error:'You do not have access to this workspace.'},{status:403});
  ...
  if(!isAdmin){
    ...
    for(const platform of actualPlatforms){const p=byPlatform.get(platform);if(!p?.can_view)return NextResponse.json({error:`You do not have View permission for ${platform}.`},{status:403});if(!p?.can_create)return NextResponse.json({error:`You do not have Create permission for ${platform}.`},{status:403});}
  }
  const approvalStatus=isAdmin?'approved':'pending';
  const {data:draft,error:draftError}=await sb.from('post_drafts').insert({user_id:user.id,brand_id:brandId,title:'Social Post',message,media_urls:mediaUrl?[mediaUrl]:[],platforms:platforms.length?platforms:actualPlatforms,account_ids:accountIds,approval_status:approvalStatus,submitted_at:new Date().toISOString()}).select('id').single();
  if(draftError)return NextResponse.json({error:draftError.message},{status:500});
  const {data:approval,error:approvalError}=await sb.from('post_approvals').insert({draft_id:draft.id,workplace_id:brandId,submitted_by:user.id,status:approvalStatus,submitted_at:new Date().toISOString()}).select('id,status').single();
  ...
  return NextResponse.json({draftId:draft.id,approval,isAdmin},{status:201});
}
