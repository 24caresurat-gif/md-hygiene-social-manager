...
export async function POST(request:Request){
  ...
  try{
    ...
    const {data:account,error}=await supabase.from('social_accounts').select('id,name,platform_account_id,access_token,refresh_token,token_expires_at,status').eq('id',accountId).eq('user_id',user.user.id).eq('platform','google_business').single();
    if(error||!account)throw new Error('Connected Google Business location not found.'); if(account.status!=='connected'||!account.access_token)throw new Error('Google Business connection is not active.');
    ...
    await supabase.from('social_posts').insert({user_id:user.user.id,social_account_id:account.id,platform:'google_business',platform_post_id:result.name||result.localPost?.name||null,message:summary,media_type:body.mediaUrl?'image':'none',status:'published',published_at:new Date().toISOString()});
