...
export async function GET(request:Request){
 ...
 const response=auth?NextResponse.json({url:oauthUrl}):NextResponse.redirect(oauthUrl);
 ...
 if(auth)response.cookies.set('google_business_user_token',auth,{httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',path:'/',maxAge:600});
 return response;
}
