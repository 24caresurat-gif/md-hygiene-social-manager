...
export async function POST(request: Request) {
  ...
  } catch (error) { const errorMessage = error instanceof Error ? error.message : 'Unable to publish to Facebook.'; if (supabase && userId && account) await supabase.from('social_posts').insert({ user_id: userId, brand_id: account.brand_id, social_account_id: account.id, platform: 'facebook', platform_post_id: null, message: message || '', link: link || null, media_type: mediaType, status: 'failed', attempted_at: attemptedAt, error_message: errorMessage } as any); return NextResponse.json({ error: errorMessage }, { status: 400 }); }
}
