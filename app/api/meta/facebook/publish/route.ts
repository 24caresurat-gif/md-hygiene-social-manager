...
export async function POST(request: Request) {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    body = {
      ...
      mediaType: String(form.get('mediaType') || 'none') as PublishBody['mediaType'],
    };
    const file = form.get('media');
    media = file instanceof File ? file : null;
  } else {
    body = (await request.json().catch(() => null)) as PublishBody | null || {};
  }
  ...
  try {
    ...
    const { data: foundAccount, error: accountError } = await supabase
      ...
      .select('id,name,platform_account_id,access_token,platform,status,brand_id')
      .eq('id', accountId)
      .eq('user_id', userId)
    ...
    if (mediaType === 'image' && media) {
      ...
      upload.append('source', media, media.name);
      upload.append('caption', message);
      upload.append('published', 'true');
      upload.append('access_token', pageToken);
      if (link) upload.append('link', link);
      formData = upload;
    } else if (mediaType === 'video' && media) {
      endpoint = `https://graph.facebook.com/v23.0/${pageId}/videos`;
      const upload = new FormData();
      upload.append('source', media, media.name);
      upload.append('description', message);
      upload.append('published', 'true');
      upload.append('access_token', pageToken);
      formData = upload;
    } else if (link) {
      (formData as URLSearchParams).set('link', link);
    }
    ...
    const { error: trackingError } = await supabase.from('social_posts').insert({
      user_id: userId,
