...
export async function POST(request: Request) {
  ...
  let supabase: ReturnType<typeof createClient> | null = null; let userId = ''; let account: Account | null = null; const attemptedAt = new Date().toISOString();
  try {
    supabase = createClient(url, anon, { global: { headers: { Authorization: `Bearer ${token}` } } }); const { data: userData, error: userError } = await supabase.auth.getUser(token); if (userError || !userData.user) throw new Error('Invalid Social Manager session.'); userId = userData.user.id;
    ...
    const postId = result.id || result.post_id || result.video_id || null; const { error: trackingError } = await supabase.from('social_posts').insert({ user_id: userId, brand_id: account.brand_id, social_account_id: account.id, platform: 'facebook', platform_post_id: postId, message, link: link || null, media_type: mediaType, status: 'published', published_at: new Date().toISOString(), attempted_at: attemptedAt, platform_response: result }); if (trackingError) console.error('social_posts tracking failed:', trackingError.message);
    return NextResponse.json({ published: true, postId, page: account.name });
  } catch (error) { const errorMessage = error instanceof Error ? error.message : 'Unable to publish to Facebook.'; if (supabase && userId && account) await supabase.from('social_posts').insert({ user_id: userId, brand_id: account.brand_id, social_account_id: account.id, platform: 'facebook', platform_post_id: null, message: message || '', link: link || null, media_type: mediaType, status: 'failed', attempted_at: attemptedAt, error_message: errorMessage }); return NextResponse.json({ error: errorMessage }, { status: 400 }); }
}
