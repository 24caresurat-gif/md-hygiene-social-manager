...
export async function GET(request: Request) {
  ...
  const expectedState = readCookie('instagram_oauth_state');
  const brandId = decodeURIComponent(readCookie('instagram_brand_id'));
  ...
  try {
    ...
    try {
      const longResponse = await fetch(`https://graph.instagram.com/access_token?grant_type=ig_exchange_token&client_secret=${encodeURIComponent(INSTAGRAM_APP_SECRET)}&access_token=${encodeURIComponent(accessToken)}`, { cache: 'no-store' });
      ...
      if (longResponse.ok && longData?.access_token) {
        ...
        if (longData.expires_in) expiresAt = new Date(Date.now() + Number(longData.expires_in) * 1000).toISOString();
      }
    } catch {}

    const profileResponse = await fetch(`https://graph.instagram.com/v23.0/me?fields=id,user_id,username,name&access_token=${encodeURIComponent(accessToken)}`, { cache: 'no-store' });
    ...
    if (!anon || !supabaseUrl) throw new Error('Supabase configuration is missing.');

    const authHeader = readCookie('sb-access-token');
    if (!authHeader) throw new Error('Login session is missing. Please reconnect while logged in.');
    const supabase = createClient(supabaseUrl, anon, { global: { headers: { Authorization: `Bearer ${authHeader}` } } });
    const { data: user, error: authError } = await supabase.auth.getUser(authHeader);
    if (authError || !user.user) throw new Error('Login session is invalid. Please reconnect.');

    const now = new Date().toISOString();
    const payload = {
      user_id: user.user.id,
      platform: 'instagram',
      name: profileData.name || profileData.username || `Instagram ${profileData.id}`,
      handle: profileData.username || null,
      platform_account_id: String(profileData.id),
    ...
    return fail(error instanceof Error ? error.message : 'Unable to connect Instagram.');
  }
}
