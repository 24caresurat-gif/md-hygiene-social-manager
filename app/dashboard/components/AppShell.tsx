...
const nav=[
  ...
  ['Settings','settings','/dashboard/settings','workspace_settings'],
] as const;
...
function Icon({name,size=18}:{name:IconName;size?:number}){
  ...
  switch(name){
    ...
    case 'settings': return <svg {...common}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.9l.1.1-1.7 1.7-.1-.1a1.7 1.7 0 0 0-1.9-.3 1.7 1.7 0 0 0-1 1.6V20h-2.4v-.1a1.7 1.7 0 0 0-1-1.6 1.7 1.7 0 0 0-1.9.3l-.1.1L8 17l.1-.1A1.7 1.7 0 0 0 8.4 15a1.7 1.7 0 0 0-1.6-1H6v-2.4h.8a1.7 1.7 0 0 0 1.6-1 1.7 1.7 0 0 0-.3-1.9L8 8.6l1.7-1.7.1.1a1.7 1.7 0 0 0 1.9.3 1.7 1.7 0 0 0 1-1.6V5H15v.7a1.7 1.7 0 0 0 1 1.6 1.7 1.7 0 0 0 1.9-.3l.1-.1 1.7 1.7-.1.1a1.7 1.7 0 0 0-.3 1.9 1.7 1.7 0 0 0 1.6 1H22V14h-.8a1.7 1.7 0 0 0-1.8 1Z"/></svg>;
...
export default function AppShell({children,title='Dashboard'}:{children:ReactNode;title?:string}){
  ...
  return <main className="app-shell"><div className={`sidebar-backdrop ${open?'show':''}`} onClick={()=>setOpen(false)}/><aside className={`sidebar ${open?'mobile-open':''}`}><div className="logo-block"><div className="logo-mark">MD</div><div className="logo-title">MD<span>HYGIENE</span><small>SOCIAL MANAGER</small></div><button className="drawer-close" onClick={()=>setOpen(false)}><Icon name="chevron" size={18}/></button></div><nav>{visibleNav.map(([label,icon,href])=><button key={label} className={`nav-item ${title===label?'active':''}`} onClick={()=>{setOpen(false);location.href=href}}><b><Icon name={icon as IconName} size={17}/></b><span>{label}</span></button>)}</nav><div className="side-user" style={{position:'relative'}}><div className="user-avatar">MD</div><div><strong>{access.role==='owner'?'Workspace Owner':access.role==='admin'?'Workspace Admin':access.role==='manager'?'Workspace Manager':'Employee'}</strong><small>{access.employee_id||email||'Account'}</small></div><button aria-label="Account menu" onClick={()=>setProfileOpen(v=>!v)}><Icon name="chevron" size={16}/></button>{profileOpen&&<div style={{position:'absolute',right:0,bottom:'58px',width:190,padding:8,border:'1px solid #e1e6eb',borderRadius:12,background:'#fff',boxShadow:'0 18px 40px rgba(15,23,42,.14)',zIndex:50}}><button onClick={signOut} style={{width:'100%',border:0,borderRadius:9,padding:'10px 12px',background:'#fff1f0',color:'#b42318',fontWeight:800,cursor:'pointer',textAlign:'left'}}>Logout</button></div>}</div></aside><section className="main-shell"><header className="main-header"><button className="menu-btn" onClick={()=>setOpen(true)} aria-label="Open navigation"><Icon name="menu" size={18}/></button><strong>{title}</strong><div className="search-box"><Icon name="search" size={16}/><span>Search anything...</span><kbd>Ctrl K</kbd></div><div className="header-actions"><button aria-label="Notifications"><Icon name="bell" size={17}/></button><select aria-label="Switch workspace" value={selected} onChange={e=>selectWorkspace(e.target.value)} style={{border:'1px solid #dfe4ea',borderRadius:10,padding:'8px 10px',background:'#fff',fontWeight:700,maxWidth:170}}>{workspaces.map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select><button className="header-profile" onClick={()=>setProfileOpen(v=>!v)}><div className="mini-logo">{selectedWorkspace?.logo_url?<img src={selectedWorkspace.logo_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover',borderRadius:10}}/>:'WS'}</div><strong>{access.role==='owner'?'Owner':access.role==='admin'?'Admin':access.role==='manager'?'Manager':'Employee'} <Icon name="chevron" size={14}/></strong></button>{profileOpen&&<div style={{position:'absolute',right:16,top:58,width:190,padding:8,border:'1px solid #e1e6eb',borderRadius:12,background:'#fff',boxShadow:'0 18px 40px rgba(15,23,42,.14)',zIndex:60}}><div style={{padding:'8px 10px',fontSize:11,color:'#667085',fontWeight:650,overflow:'hidden',textOverflow:'ellipsis'}}>{email||access.employee_id||'Account'}</div><button onClick={signOut} style={{width:'100%',border:0,borderRadius:9,padding:'10px 12px',background:'#fff1f0',color:'#b42318',fontWeight:800,cursor:'pointer',textAlign:'left'}}>Logout</button><button onClick={backToWorkspaces} style={{width:'100%',border:0,borderRadius:9,padding:'10px 12px',background:'#f7f9fb',color:'#17202b',fontWeight:800,cursor:'pointer',textAlign:'left',marginTop:5}}>Workspaces</button></div>}</div></header><div className="page-content">{children}</div></section></main>;
}
