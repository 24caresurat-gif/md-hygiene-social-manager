...
export default function BrandSelector({ value, onChange }: { value: string; onChange: (id: string) => void }) {
  ...
  return (
    <>
      <style jsx>{`
        .brandSelector { width:min(390px,100%); background:#fff; border:1px solid #dce6ea; border-radius:16px; padding:14px; box-shadow:0 10px 28px rgba(15,23,42,.07); }
        ...
        @media(max-width:760px) { .brandSelector { width:100%; } .select { min-height:46px; font-size:14px; } }
      `}</style>
      <div className="brandSelector" aria-label="Business workspace selector">
        <div className="title">
          <span className="icon">▦</span>
          <div><strong>Select Business / Brand</strong><small>Dashboard, accounts & publishing</small></div>
        </div>
        <select className="select" value={value} onChange={(e) => onChange(e.target.value)} disabled={loading} aria-label="Select business or brand">
          <option value={ALL_BRANDS_ID}>{loading ? 'Loading businesses…' : 'All Businesses'}</option>
          {brands.map((b) => <option value={b.id} key={b.id}>{b.name}</option>)}
        </select>
        {error ? <small className="error">{error}</small> : null}
        <a className="manage" href="/dashboard/brands">＋ Create / Manage Business Cards</a>
      </div>
    </>
  );
}
