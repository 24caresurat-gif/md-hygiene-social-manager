...
export default function DashboardPage() {
  ...
  return <main className="app-shell">
    <aside className="sidebar"><div className="logo-block"><div className="logo-mark">MD</div><div className="logo-title">MD<span>HYGIENE</span><small>SOCIAL MANAGER</small></div></div>
      <nav>{nav.map(([label,icon,href])=><button key={label} className={label==='Dashboard'?'nav-item active':'nav-item'} onClick={()=>href!=='#'&&(window.location.href=href)}><b>{icon}</b><span>{label}</span></button>)}</nav>
      ...
      <div className="side-user"><div className="user-avatar">MD</div><div><strong>MD Hygiene</strong><small>Admin</small></div><button onClick={signOut}>⌄</button></div><div className="dark-toggle"><span>◐ &nbsp; Dark Mode</span><i/></div>
    </aside>
    <section className="main-shell"><header className="main-header"><button className="menu-btn">☰</button><strong>Dashboard</strong><div className="search-box">⌕ <span>Search anything...</span><kbd>Ctrl K</kbd></div><div className="header-actions"><span>♧</span><span className="notification">♢<i>3</i></span><div className="header-profile"><div className="mini-logo">MD</div><strong>MD Hygiene</strong><span>⌄</span></div></div></header>
